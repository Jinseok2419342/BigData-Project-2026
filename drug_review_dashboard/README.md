...
| 문서 | 내용 |
|---|---|
| [보고서.md](보고서.md) | 최종 보고서 — 문제정의·EDA·특성·모델·결론·한계 |
| [문제정의.md](문제정의.md) | 1차 작업에서 작성한 문제정의 (교수님 점검본) |
| [발표자료.md](발표자료.md) / [발표_슬라이드.html](발표_슬라이드.html) | 5분 발표 슬라이드 (실제 덱은 `.html`, 브라우저 발표·인쇄용 `발표_슬라이드.pdf` 동봉) |
| [docs/특성_엔지니어링.md](docs/특성_엔지니어링.md) | **EDA 발견 → 특성** 매핑 상세표 (특성별 동기·계산·유용성·누수 여부) |
| [docs/코드_워크스루.md](docs/코드_워크스루.md) | 파일·함수별 "이 코드가 뭘 하는지" 설명 |
| [docs/발표_준비.md](docs/발표_준비.md) | 아키텍처 요약·핵심 포인트·예상 Q&A 11문항 |
| [docs/발표_대본.md](docs/발표_대본.md) | 5분 발표 대본 (슬라이드별·시간 배분) |
| [docs/발표_치트시트.md](docs/발표_치트시트.md) | **발표 당일 손에 들 압축본** — 세팅·5분 멘트·데모 입력·코드 Q&A 9문항 |
...
| **4. 데이터셋 라이선스 명확화** | "UCI 원본 라이선스 설명은 일부 부정확할 가능성이 있습니다... 정확한 라이선스 문구를 정리하는 것이 좋습니다." | 사용 데이터가 UCI **Drug Review Dataset (Drugs.com)** (Gräßer et al. 2018, UCI #462)임을 확인(파일명 `drugsComTrain_raw.csv`로 검증, Druglib.com 데이터셋(UCI #461)과 구분). UCI 기준 라이선스는 **CC BY 4.0**, Kaggle 재배포본은 더 엄격한 **CC BY-NC-SA 4.0** → 엄격한 쪽을 따르도록 문서를 통일하여 법적/학술적 리스크를 제거함. |
...
> **데이터 출처/라이선스**: UCI Drug Review Dataset (Drugs.com) — UCI ML Repository #462 (Gräßer et al. 2018), **CC BY 4.0**.
> 사용본은 Kaggle 재배포 [`jessicali9530/kuc-hackathon-winter-2018`](https://www.kaggle.com/datasets/jessicali9530/kuc-hackathon-winter-2018)이며 Kaggle 페이지는 더 엄격한 **CC BY-NC-SA 4.0**을 명시 → 본 프로젝트는 엄격한 쪽을 따라 **비상업적·수업용**으로만 사용하고 원본 CSV는 저장소에 포함하지 않습니다(출처 URL만 표기).
...
약한 라벨은 심각/증상 키워드 수와 낮은 평점으로 정의되므로, 이 컬럼들을 그대로 학습에 넣으면 모델이 라벨 규칙을 복원해 비현실적 성능(Acc~99%)이 나온다. 따라서 라벨 정의 컬럼(`severe_keyword_count`, `symptom_keyword_count`, `low_rating_flag`)을 학습 특성에서 제외(`MODEL_FEATURES`)하고 리뷰 원문 TF-IDF + 비누수 특성만으로 학습해 **정직한 성능(F1≈0.90)** 을 보고한다. 자세한 내용은 `보고서.md` 3.3절·4장 참고.
